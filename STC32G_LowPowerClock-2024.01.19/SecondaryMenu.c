...
void UIData_fill(u16 a,u8 b)
{
    if(dat00[0x10] == 0x0008 && dat00[0x009E] == 1)
    {
			if(b==0)
			{
				OLED_AlarmClock_ZQ((u8)a);
			}
			else
			{
			  OLED_AlarmClock_ZH((u8)a);
			}
    }	
    else if(dat00_max[a] & 0x0040)
    {
...
void UI_SeMenu_Cursor_num(char num,u8 x,u8 yy1,u8 yy2,u8 a)
{
    ...
    {
        if(dat00[0x10] == 1 | dat00[0x10] == 2 | dat00[0x10] == 4 | dat00[0x10] == 3 | dat00[0x10] == 0x0B | dat00[0x10] == 0x0D | dat00[0x10] == 0x07 | (dat00[0x10] == 0x0008 && dat00[0x009E] == 2))//设置哪个可以进入二级菜单
        {
            UIData_fill(UI_S_F2(num),0);
            address2 = 0;
            address = 0x60;
        }
        ...
        else if(dat00[0x10] == 0x0008 && dat00[0x009E] == 1)
        {
					  UIData_fill(address3/8,1);
            address = 0x60;
            address2 = 0;
        }				
    }
...
{
    UI_SeMenu_Cursor_num(num,0,yy1,yy2,a);
    UI_SeMenu_Cursor_num(num,64,yy1,yy2,a);
}
...
{
    u8 yy = 0;
    UI_SeMenu_Cursor_num(-1,64,0,yy,0);
    yy =  24;
    UI_SeMenu_Cursor_num(0,64,yy,yy,1);
    yy = 48 ;
    UI_SeMenu_Cursor_num(1,64,yy,yy,1);
