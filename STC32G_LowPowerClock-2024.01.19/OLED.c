...
void SPI_DMA_delay()
{
	  ...
	  while(SPI_DMA_buy) //清0 SPIF和WCOL标志
		{
    _nop_();
    _nop_();
    _nop_();
...
void LCD_Writ_Bus(u8 out)
{
	  u16 i = 500;
		SPI_DMA_delay();
    SPDAT = out;
    while(((SPSTAT & 0xC0) == 0)); //清0 SPIF和WCOL标志
    SPSTAT = 0xC0;
}
...
void OLED_Init(void)
{
    ...
    OLED_WrCmd(0x80);//--set divide ratio, Set Clock as 100 Frames/Sec	  设置分频比，将时钟设置为100帧/秒
    ...
    OLED_WrCmd(0x10);//--set(0x10) (0x14)打开
...
void OLED_8X8X3_MOV(u8 x,u8 y,u16 adde,u8 devia1,u8 devia2)
{
    unsigned char i = 0;
    unsigned char j = 0;
    if(devia1 > 24) devia1 =0;
    if(devia2 > 24) devia2 =24;
    if(x >= 244)
		{
			  devia1 = 256 - x; 
				x = 0;  
...
void OLED_16X16_MOV(u8 x,u8 y,u8 adde,u8 devia1,u8 devia2)
{
    ...
    if(devia1 > 16) devia1 =0;
    if(devia2 > 16) devia2 =16;
    if(x >= 248)
		{
			  devia1 = 256 - x; 
				x = 0;  
...
void OLED_AlarmClock_ZQ(u8 a)
{
    ...
    u32 dat=0;
	  ...
	  if(dat & 0x00000001) k=8;
    for(j = 0; j <2 ; j++)
    {
        for(i = 0; i <16 ; i++)
        {
            UI_dat[j][i] = DSW_1[j+k][i];
        }
    }
    ...
    for(j = 6; j <8 ; j++)
    {
        ...
        for(i = 0; i <8 ; i++)
        {
            UI_dat[2][i+4] = F8x16[a][i];
        }	

        for(i = 0; i <8 ; i++)
        {
            UI_dat[3][i+4] = F8x16[a][i+8];
        }		
...
void OLED_Mean2_fill_num_10X(u8 k,u8 a)
{
    OLED_Mean2_fill_num((u8)(a+1),(u8)(k % 10),F8x16);
    OLED_Mean2_fill_num(a,(u8)(k / 10),F8x16);  
}

void OLED_AlarmClock_ZH(u8 a)
{
    u32 dat = 0;
	  u8 k=0;
	  ...
	  k = (u8)(dat & 0x0000003f);
    OLED_Mean2_fill_num_10X(k,6); 
    OLED_Mean2_fill_num(5,20,F8x16);
	
	
	
	  dat = dat>>6;
	  k = (u8)(dat & 0x0000003f);
    OLED_Mean2_fill_num_10X(k,3);  
    OLED_Mean2_fill_num(2,20,F8x16);
	
	
	  dat = dat>>6;
	  k = (u8)(dat & 0x0000001f);	
    OLED_Mean2_fill_num_10X(k,0); 	
...
void IU_Battery_Icon_S(u8 x,u8 y)
{
    u16 a = 0;
    u8 j = 0;
    u8 i;
...
void IU_Battery_cheek_renovate()
{
    ...
    if(dat00[0x00E4]>=100)
    {
        ZF_8X16(56,5,(u8)((dat00[0x00E4]%1000)/100));
    }
    else
    {
        ZF_8X16(56,5,10);
    ...
    if(dat00[0x00E4]>=10)
    {
        ZF_8X16(64,5,(u8)((dat00[0x00E4]%100)/10));
    }
    else
    {
        ZF_8X16(64,5,10);
    }
    ZF_8X16(72,5,(u8)(dat00[0x00E4]%10));
